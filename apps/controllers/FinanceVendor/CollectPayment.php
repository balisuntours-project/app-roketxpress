...
class CollectPayment extends CI_controller {
	...
	public function getDataCollectPayment(){
		...
		$this->load->model('FinanceVendor/ModelCollectPayment');
		...
		$dataTable				=	$this->ModelCollectPayment->getDataCollectPayment($page, 25, $idVendor, $startDate, $endDate, $collectStatus, $settlementStatus, $viewRequestOnly);
		$totalSettlementRequest	=	$this->ModelCollectPayment->getTotalSettlementRequest();
		$urlExcelCollectPayment	=	BASE_URL."financeVendor/collectPayment/excelCollectPayment/".base64_encode(encodeStringKeyFunction($idVendor."|".$startDate."|".$endDate."|".$collectStatus."|".$settlementStatus."|".$viewRequestOnly, DEFAULT_KEY_ENCRYPTION))."/token?token=".$this->newToken;
	...
	public function approveRejectCollectPaymentSettlement(){
		...
		$idReservationDetails	=	$this->ModelReservation->getIdReservationDetailsVendor($idReservation, $idVendor, $dateCollectDB);
		...
		if(PRODUCTION_URL){
			...
			try {
				...
				$reference				=	$database->getReference(FIREBASE_RTDB_MAINREF_NAME."unprocessedFinanceVendor/collectPayment")
											->set([
	...
	public function excelCollectPayment($encryptedVar){
		...
		$dataTable		=	$this->ModelCollectPayment->getDataCollectPayment(1, 999999, $idVendor, $startDate, $endDate, $collectStatus, $settlementStatus, $viewRequestOnly);
		...
		$sheet->setCellValue('A2', 'Vendor Collect Payment Report');
		$sheet->getStyle('A1:A2')->getFont()->setBold( true );
		$sheet->getStyle('A1:A2')->getAlignment()->setHorizontal('center');
		$sheet->mergeCells('A1:L1');
		$sheet->mergeCells('A2:L2');
		
		$sheet->setCellValue('A4', 'Vendor Name : '.$vendorName);
		...
		$sheet->setCellValue('B9', 'Vendor Name');
		...
		$filename		=	'ExcelVendorFinanceCollectPayment_'.$vendorName.'_'.$startDateStr.'_'.$endDateStr;
