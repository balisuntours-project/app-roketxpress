...
if (collectPaymentFunc == null){
	var collectPaymentFunc	=	function(){
...
$('#startDate, #endDate, #optionDriver, #optionCollectStatus, #optionSettlementStatus').off('change');
$('#startDate, #endDate, #optionDriver, #optionCollectStatus, #optionSettlementStatus').on('change', function(e) {
	getDataCollectPayment();
});
...
$("#checkboxViewSettlementRequestOnly").on('click',function(e) {
	...
	if(checked){
		$("#startDate, #endDate, #optionDriver, #optionCollectStatus, #optionSettlementStatus").attr("disabled", true);
		$("#btnShowAllSetttlementRequest").addClass("d-none");
	} else {
		$("#startDate, #endDate, #optionDriver, #optionCollectStatus, #optionSettlementStatus").attr("disabled", false);
...
function getDataCollectPayment(page = 1){
	...
	$.ajax({
		type: 'POST',
		url: baseURL+"financeDriver/collectPayment/getDataCollectPayment",
			...
			if(data.length === 0){
				...
				$.each(data, function(index, array) {
					...
					rows			+=	"<tr>"+
											...
											"</td>"+
											"<td>"+
												"["+array.DRIVERTYPE+"] "+array.DRIVERNAME+"<br/>"+
...
function getMoreDetailsCollectPayment(idCollectPayment){
	...
	$.ajax({
		type: 'POST',
		url: baseURL+"financeDriver/collectPayment/getDetailCollectPayment",
		contentType: 'application/json',
		dataType: 'json',
		cache: false,
		data: mergeDataSend(dataSend),
		...
		success:function(response){
			...
			if(response.status == 200){
				...
				$("#driverNameTypeStr").html("["+detailCollectPayment.DRIVERTYPE+"] "+detailCollectPayment.DRIVERNAME);
...
function createUploaderSettlementReceipt(idCollectPayment, idCollectPaymentHistory){
	...
	$("#uploaderSettlementReceipt").uploadFile({
		url: baseURL+"financeDriver/collectPayment/uploadSettlementReceipt/"+idCollectPayment+"/"+idCollectPaymentHistory,
		multiple:false,
		dragDrop:false,
...
$('#confirmBtn').on('click', function(e) {
	...
	$.ajax({
		type: 'POST',
		url: baseURL+"financeDriver/collectPayment/"+functionUrl,
		contentType: 'application/json',
		dataType: 'json',
		data: mergeDataSend(dataSend),
		beforeSend:function(){
