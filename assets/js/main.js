...
function generateTotalUnprocessReservarionElem(totalUnprocessedReservation){
	...
	if($("#menuRV").length > 0){
		...
		if(totalUnprocessedReservation > 0){
			$("#menuRV a").append('<span class="badge badge-primary badge-pill ml-auto mr-4" id="containerUnprocessReservationCounter" data-toggle="tooltip" data-original-title="Total unprocessed reservation" data-placement="right">'+numberFormat(totalUnprocessedReservation)+'</span>');
			$('[data-toggle="tooltip"]').tooltip();
		}
	}
}

function generateTotalReservationElem(totalUnprocessedReservation){
	totalUnprocessedReservation	=	totalUnprocessedReservation * 1;
	
	if($("#groupMenuReservation").length > 0){
		$("#containerReservationCounter").remove();
		if(totalUnprocessedReservation > 0){
			$("#groupMenuReservation").after('<span class="badge badge-success badge-pill ml-auto mr-4" id="containerReservationCounter" data-toggle="tooltip" data-original-title="Total unprocessed reservation" data-placement="right">'+numberFormat(totalUnprocessedReservation)+'</span>');
			$('[data-toggle="tooltip"]').tooltip();
		}
	}
}

function generateTotalUnreadThreadReconfirmation(totalUnreadThreadReconfirmation){
	totalUnreadThreadReconfirmation	=	totalUnreadThreadReconfirmation * 1;
	if($("#menuRRC").length > 0){
		$("#containerUnreadThreadReconfirmation").remove();
		if(totalUnreadThreadReconfirmation > 0){
			$("#menuRRC a").append('<span class="badge badge-info badge-pill ml-auto mr-4" id="containerUnreadThreadReconfirmation" data-toggle="tooltip" data-original-title="Total unread conversation" data-placement="right">'+numberFormat(totalUnreadThreadReconfirmation)+'</span>');
			$('[data-toggle="tooltip"]').tooltip();
		}
	}
}
...
function generateTotalReimbursementElem(totalReimbursement){
	...
	if($("#menuFRBM").length > 0){
		$("#containerReimbursementCounter").remove();
		if(totalReimbursement > 0){
			$("#menuFRBM a").append('<span class="badge badge-primary badge-pill ml-auto" id="containerReimbursementCounter" data-toggle="tooltip" data-original-title="Total new reimbursement request" data-placement="right">'+numberFormat(totalReimbursement)+'</span>');
			$('[data-toggle="tooltip"]').tooltip();
		}
	}
}

function generateTotalFinanceElem(totalReimbursement){
	var totalFinance		=	0;
		totalReimbursement	=	totalReimbursement * 1;
	
	if($("#menuFRBM").length > 0){
		totalFinance	+=	totalReimbursement;
	}
		
	if($("#groupMenuFinance").length > 0){
		$("#containerFinanceCounter").remove();
		if(totalFinance > 0){
			$("#groupMenuFinance").after('<span class="badge badge-primary badge-pill ml-auto mr-4" id="containerFinanceCounter" data-toggle="tooltip" data-original-title="Total new finance request" data-placement="right">'+numberFormat(totalFinance)+'</span>');
			$('[data-toggle="tooltip"]').tooltip();
		}
	}
}

function generateTotalWithdrawalDriverRequestElem(totalWithdrawalRequest){
	totalWithdrawalRequest	=	totalWithdrawalRequest * 1;
	if($("#menuFDRPD").length > 0){
		$("#containerWithdrawalDriverRequestCounter").remove();
		if(totalWithdrawalRequest > 0){
			$("#menuFDRPD a").append('<span class="badge badge-primary badge-pill ml-auto" id="containerWithdrawalDriverRequestCounter" data-toggle="tooltip" data-original-title="Total new additional cost request" data-placement="right">'+numberFormat(totalWithdrawalRequest)+'</span>');
			$('[data-toggle="tooltip"]').tooltip();
		}
	}
}

function generateTotalAdditionalCostDriverElem(totalAdditionalCost){
	totalAdditionalCost	=	totalAdditionalCost * 1;
	if($("#menuFDAC").length > 0){
		$("#containerAdditionalCostDriverCounter").remove();
		if(totalAdditionalCost > 0){
			$("#menuFDAC a").append('<span class="badge badge-primary badge-pill ml-auto" id="containerAdditionalCostDriverCounter" data-toggle="tooltip" data-original-title="Total new additional cost request" data-placement="right">'+numberFormat(totalAdditionalCost)+'</span>');
			$('[data-toggle="tooltip"]').tooltip();
		}
	}
}

function generateTotalAdditionalIncomeDriverElem(totalAdditionalIncome){
	totalAdditionalIncome	=	totalAdditionalIncome * 1;
	if($("#menuFDAI").length > 0){
		$("#containerAdditionalIncomeDriverCounter").remove();
		if(totalAdditionalIncome > 0){
			$("#menuFDAI a").append('<span class="badge badge-primary badge-pill ml-auto" id="containerAdditionalIncomeDriverCounter" data-toggle="tooltip" data-original-title="Total new additional income request" data-placement="right">'+numberFormat(totalAdditionalIncome)+'</span>');
			$('[data-toggle="tooltip"]').tooltip();
		}
	}
}

function generateTotalCollectPaymentDriverElem(totalCollectPayment){
	totalCollectPayment	=	totalCollectPayment * 1;
	if($("#menuFDRCP").length > 0){
		$("#containerCollectPaymentDriverCounter").remove();
		if(totalCollectPayment > 0){
			$("#menuFDRCP a").append('<span class="badge badge-primary badge-pill ml-auto" id="containerCollectPaymentDriverCounter" data-toggle="tooltip" data-original-title="Total new collect payment settlement request" data-placement="right">'+numberFormat(totalCollectPayment)+'</span>');
			$('[data-toggle="tooltip"]').tooltip();
		}
	}
}

function generateTotalLoanPrepaidCapitalDriverElem(totalLoanPrepaidCapital){
	totalLoanPrepaidCapital	=	totalLoanPrepaidCapital * 1;
	if($("#menuFDRLP").length > 0){
		$("#containerLoanPrepaidCapitalDriverCounter").remove();
		if(totalLoanPrepaidCapital > 0){
			$("#menuFDRLP a").append('<span class="badge badge-primary badge-pill ml-auto" id="containerLoanPrepaidCapitalDriverCounter" data-toggle="tooltip" data-original-title="Total new loan/prepaid capital request" data-placement="right">'+numberFormat(totalLoanPrepaidCapital)+'</span>');
			$('[data-toggle="tooltip"]').tooltip();
		}
	}
}

function generateTotalFinanceDriverElem(totalWithdrawalRequest, totalAdditionalCost, totalCollectPayment, totalLoanPrepaidCapital, totalAdditionalIncome){
	var totalFinanceDriver	=	0;
	totalWithdrawalRequest	=	totalWithdrawalRequest * 1;
	totalAdditionalCost		=	totalAdditionalCost * 1;
	totalAdditionalIncome	=	totalAdditionalIncome * 1;
	totalCollectPayment		=	totalCollectPayment * 1;
	...
	if($("#menuFDRPD").length > 0){
		totalFinanceDriver	+=	totalWithdrawalRequest;
	}
	
	if($("#menuFDAC").length > 0){
		totalFinanceDriver	+=	totalAdditionalCost;
	}
	
	if($("#menuFDAI").length > 0){
		totalFinanceDriver	+=	totalAdditionalIncome;
	}
	
	if($("#menuFDRCP").length > 0){
		totalFinanceDriver	+=	totalCollectPayment;
	}
	
	if($("#menuFDRLP").length > 0){
		totalFinanceDriver	+=	totalLoanPrepaidCapital;
	}
	
	if($("#groupMenuFinance-Driver").length > 0){
		$("#containerFinanceDriverCounter").remove();
		if(totalFinanceDriver > 0){
			$("#groupMenuFinance-Driver").after('<span class="badge badge-primary badge-pill ml-auto mr-4" id="containerFinanceDriverCounter" data-toggle="tooltip" data-original-title="Total new finance (driver) request" data-placement="right">'+numberFormat(totalFinanceDriver)+'</span>');
			$('[data-toggle="tooltip"]').tooltip();
		}
	}
}

function generateTotalWithdrawalVendorRequestElem(totalWithdrawalRequest){
	totalWithdrawalRequest	=	totalWithdrawalRequest * 1;
	if($("#menuFVRFV").length > 0){
		$("#containerWithdrawalVendorRequestCounter").remove();
		if(totalWithdrawalRequest > 0){
			$("#menuFVRFV a").append('<span class="badge badge-primary badge-pill ml-auto" id="containerWithdrawalVendorRequestCounter" data-toggle="tooltip" data-original-title="Total new additional cost request" data-placement="right">'+numberFormat(totalWithdrawalRequest)+'</span>');
			$('[data-toggle="tooltip"]').tooltip();
		}
	}
}

function generateTotalCollectPaymentVendorElem(totalCollectPayment){
	totalCollectPayment	=	totalCollectPayment * 1;
	if($("#menuFVRCP").length > 0){
		$("#containerCollectPaymentVendorCounter").remove();
		if(totalCollectPayment > 0){
			$("#menuFVRCP a").append('<span class="badge badge-primary badge-pill ml-auto" id="containerCollectPaymentVendorCounter" data-toggle="tooltip" data-original-title="Total new collect payment settlement request" data-placement="right">'+numberFormat(totalCollectPayment)+'</span>');
			$('[data-toggle="tooltip"]').tooltip();
		}
	}
}
...
function openListNotification(){
	...
	$('.adomx-dropdown-menu.dropdown-menu-notifications.show').removeClass('show');
}
	
function openUserProfileSetting(){
	getViewURL('userProfileSetting', 'UPS');
	$('.adomx-dropdown.col-auto.show').removeClass('show');
...
function maskNumberInput(
    ...
    $input.on("keyup", function (event) {
        ...
        var $this = $(this),
            ...
            padzeroleft = $this.hasClass("padzeroleft") ? true : false,
            decimalInput = $this.hasClass("decimalInput"),
            input = $this.val(),
            input = input.replace(/[^0-9.]/g, '');

        if (!decimalInput) {
            input = input < minValue ? minValue : padzeroleft ? input : parseInt(input, 10);
            input = maxValue != false && input > maxValue ? maxValue : padzeroleft ? input : parseInt(input, 10);
        }
...
function toggleSlideContainer(leftContainer, rightContainer) {
	if ($("#"+leftContainer).hasClass('show')) {
		$("#"+leftContainer).find(".box, .row, .nav").addClass('d-none');
		$("#"+leftContainer).removeClass('show').addClass('hide');
		$("#"+rightContainer).removeClass('hide').addClass('show');
		$("#"+rightContainer).find(".box, .row, .nav").removeClass('d-none');
	} else {
		$("#"+rightContainer).find(".box, .row, .nav").addClass('d-none');
		$("#"+rightContainer).removeClass('show').addClass('hide');
		$("#"+leftContainer).removeClass('hide').addClass('show');
		$("#"+leftContainer).find(".box, .row, .nav").removeClass('d-none');
	}
}
...
function generateCounterField(elemNameID, defaultValue = 0, minValue = 0, maxValue = 10){
	var disabledMin	=	defaultValue == minValue ? 'disabled="disabled"' : '',
		disabledMax	=	defaultValue == maxValue ? 'disabled="disabled"' : '';
	return '<div class="input-group">'+
                '<span class="input-group-prepend">'+
                    '<button type="button" class="btn btn-outline-secondary btn-number" '+disabledMin+' data-type="minus" data-field="'+elemNameID+'">'+
                        '<span class="fa fa-minus"></span>'+
                    '</button>'+
                '</span>'+
                '<input type="text" name="'+elemNameID+'" id="'+elemNameID+'" class="form-control form-control-sm input-number text-right" value="'+defaultValue+'" min="'+minValue+'" max="'+maxValue+'">'+
                '<span class="input-group-append">'+
                    '<button type="button" class="btn btn-outline-secondary btn-number" '+disabledMax+' data-type="plus" data-field="'+elemNameID+'">'+
...
function onChangeDateInputFilter(elementObj){
	let dateValue	=	elementObj.val(),
		dateIcon	=	elementObj.attr('aria-describedby'),
		elemDateIcon=	$("#"+dateIcon).find('i');

	if(dateValue != "") {
		elemDateIcon.removeClass('fa-calendar').addClass('fa-times');
	} else {
		elemDateIcon.removeClass('fa-times').addClass('fa-calendar');
	}

	$('.iconInputDate').off('click');
	$('.iconInputDate').on('click', function(e) {
		let elemDateInput	=	$(this).closest('.input-group-append').prev('input'),
			dateValue		=	elemDateInput.val(),
			elemDateIcon	=	$(this).find('i');

		if(dateValue != ""){
			elemDateInput.val("").trigger('change');
		}
	});
}